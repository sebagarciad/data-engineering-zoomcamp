...
@click.option('--chunksize', default=100000, type=int, help='Size of each chunk to ingest')
def run(pg_user, pg_pass, pg_host, pg_port, pg_db, target_table, year, month, chunksize):
    ...
    for df_chunk in tqdm(df_iter, desc=f'Ingesting {year}-{month:02d} data'):
        if first:
            # Create the table schema (no data)
